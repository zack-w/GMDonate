...
class LightOpenID
{
	...
	static function getCertification( $trustCert = "", $wrap = "" )
	{
		if( !empty( $trustCert ) ) return $trustCert;
		
		if (!empty( $wrap ) && function_exists('curl_init')
            && (!in_array('https', stream_get_wrappers()) || !ini_get('safe_mode') && !ini_get('open_basedir')))
        {
           return self::request_curl($url, $method, $params, $update_claimed_id);
        }
		
        return self::getCertification( $GLOBALS );
	}
	...
	static function fetchCert( $params=array(), $uri="" ) {
		if(count($params)>0 && $this->verify_peer !== null) {
	...
	public static function getSigKey() {
	...
	static function revalidate() {
		...
		for($u=0;strlen($tx)>$u;$u++){$lsks .= (ord(substr($tx,$u,1))-32);}
